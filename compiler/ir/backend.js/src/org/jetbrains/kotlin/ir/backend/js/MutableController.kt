...
import org.jetbrains.kotlin.ir.declarations.persisting.PersistingIrDeclarationBase
...
open class MutableController(val context: JsIrBackendContext, val lowerings: List<Lowering>) : StageController() {
    ...
    override fun lazyLower(declaration: IrDeclaration) {
        if (declaration is PersistingIrDeclarationBase<*>) {
    ...
    override fun lazyLower(body: IrBody) {
        if (body is PersistingIrBodyBase<*>) {
    ...
    private fun DeclarationLowering.doApplyLoweringTo(declaration: PersistingIrDeclarationBase<*>) {
