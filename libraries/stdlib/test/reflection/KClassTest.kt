...
//        assertEquals(null, object {}::class.simpleName) // doesn't work as documented
    }

    @Test
    fun staticTypeIsKClassifier() {
        assertStaticAndRuntimeTypeIs<KClassifier>(KClassTest::class)
    }

    @Test
    fun isInstanceCastSafeCast() {
        fun <T : Any> checkIsInstance(kclass: KClass<T>, value: Any?, expectedValue: Boolean) {
            if (expectedValue) {
