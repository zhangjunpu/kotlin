...
class CoroutineLibraryAgent2Proxy(private val executionContext: DefaultExecutionContext) :
    ...
    private fun mapToCoroutineInfoData(mirror: MirrorOfCoroutineInfo): CoroutineInfoData? {
        ...
        val stackFrames = findStackFrames(stackTrace zip baseContinuationImpls)
    ...
    private fun findStackFrames(
        frames: List<Pair<StackTraceElement, MirrorOfBaseContinuationImpl?>>,
    ): CoroutineStackFrames {
        val index = frames.indexOfFirst { it.first.isCreationSeparatorFrame() }
        ...
        val restoredStackFrames = restoredStackTraceElements.map {
            val variables = it.second?.spilledValues(executionContext) ?: emptyList()
            SuspendCoroutineStackFrameItem(it.first, locationCache.createLocation(it.first), variables)
        }
        val creationStackFrames = frames.subList(index + 1, frames.size).mapIndexed { ix, it ->
            CreationCoroutineStackFrameItem(it.first, locationCache.createLocation(it.first), ix == 0)
        }
