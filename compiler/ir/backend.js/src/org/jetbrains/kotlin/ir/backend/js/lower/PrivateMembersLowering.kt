...
class PrivateMembersLowering(val context: JsIrBackendContext) : DeclarationTransformer {
    ...
    private fun transformMemberToStaticFunction(function: IrSimpleFunction): IrSimpleFunction? {
        ...
        val staticFunction = context.declarationFactory.buildFun {
            updateFrom(function)
            name = function.name
        ...
        function.valueParameters.forEach {
            ...
            parameterMapping[it]?.apply {
                it.defaultValue?.let { originalDefault ->
                    defaultValue = context.declarationFactory.createExpressionBody(it.startOffset, it.endOffset) {
        ...
        function.body?.let {
            staticFunction.body = when (it) {
                is IrBlockBody -> context.declarationFactory.createBlockBody(it.startOffset, it.endOffset) {
                    statements += (it.copyWithParameters() as IrBlockBody).statements
                }
                is IrExpressionBody -> context.declarationFactory.createExpressionBody(it.startOffset, it.endOffset) {
